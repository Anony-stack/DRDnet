...
from utils.metrics import R1_mAP_eval,R1_mAP_eval_LTCC, R1_mAP_eval_Celeb
...
def do_train(cfg,
    ...
    # elif cfg.DATA.DATASET == 'celeb_light':
    #     evaluator = R1_mAP_eval_Celeb(dataset.num_query_imgs, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)
    else:
    ...
    for epoch in range(cfg.TRAIN.START_EPOCH, epochs + 1):
        ...
        for idx, data in enumerate(train_loader):
            ...
            samples, targets, camids,_, clothes, masks , captions, person_captions= data
            ...
            with amp.autocast(enabled=True):
                score, feat, loss_m , loss_clothes_text, loss_person_text= model(samples,  captions, person_captions, targets)  
                score_mask, feat_mask, loss_m_mask , loss_clothes_text_mask, loss_person_text_mask= model(masks,  captions, person_captions, targets)  

            etime = time.time()

            loss = loss_fn(score, feat, targets, camids)
            loss_mask = loss_fn(score_mask, feat_mask, targets, camids) #important
            
            loss = (loss+loss_m)*1+(loss_mask+loss_m_mask)*1 + loss_clothes_text*1+ loss_person_text*1 
        ...
        if epoch % eval_period == 0:
            ...
            if cfg.DATA.DATASET == 'prcc':
                ...
                rank1, mAP= test(cfg, model, evaluator_diff, val_loader, logger, device,epoch, rank_writer, mAP_writer)
                ...
                rank1, mAP = test(cfg, model, evaluator, val_loader, logger, device,epoch,rank_writer, mAP_writer)
            is_best = rank1 > best_rank1
            if is_best:
                best_rank1 = rank1
                best_epoch = epoch
                best_mAP = mAP
                logger.info("==> Best Rank-1 {:.1%}, mAP {:.2%}, achieved at epoch {}".format(best_rank1, best_mAP, best_epoch))
...
def test(cfg, model, evaluator, val_loader, logger, device, epoch=None, rank_writer=None, mAP_writer=None,test=False,cc=False):
    ...
    for n_iter, (imgs, pids, camids, clothes_id, clothes_ids,masks,captions,person_captions) in enumerate(val_loader):
        with torch.no_grad():
            imgs = imgs.to(device)
            masks = masks.to(device)
            captions = captions.to(device)
            person_captions = person_captions.to(device)
            feat = model(imgs,captions,person_captions)  #1111
