...
class PRCC(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        #加入文本
        self.text_dir = osp.join(self.dataset_dir, "descriptions_clothes.json")
        self.person_text_dir = osp.join(self.dataset_dir, "descriptions_person.json")
        with open(self.text_dir, 'r', encoding='utf-8') as file:
            text = json.load(file)
        self.text = text
        with open(self.person_text_dir, 'r', encoding='utf-8') as file:
            person_text = json.load(file)
    ...
    def _process_dir_train(self, dir_path):
        ...
        for pdir in pdirs:
            ...
            img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
            for img_dir in img_dirs:
                caption = self.text[img_dir]
                person_caption = self.person_text[img_dir]
                cam = osp.basename(img_dir)[0] # 'A' or 'B' or 'C'
                label = pid2label[pid]
                camid = cam2label[cam]
                ...
                dataset.append((img_dir, label, camid, clothes_id,caption, person_caption))
            ...
            for pdir in pdirs:
                ...
                for img_dir in img_dirs:
                    ...
                    if cam == 'A':
                        ...
                        gallery_dataset.append((img_dir, pid, camid, clothes_id,caption, person_caption))
                    elif cam == 'B':
                        clothes_id = pid2label[pid] * 2
                        query_dataset_same_clothes.append((img_dir, pid, camid, clothes_id,caption, person_caption))
                    else:
                        clothes_id = pid2label[pid] * 2 + 1
                        query_dataset_diff_clothes.append((img_dir, pid, camid, clothes_id,caption, person_caption))

        pid2imgidx = {}

        for idx, (img_dir, pid, camid, clothes_id,caption, person_caption) in enumerate(gallery_dataset):
...
if __name__ == '__main__':
    prcc=PRCC(root='/the/path/to/your/data')